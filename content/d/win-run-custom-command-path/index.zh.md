...
```text
pathhereadd.cmd       把当前目录（或参数指定目录）加入用户 PATH
pathhereremove.cmd    把当前目录（或参数指定目录）从用户 PATH 移除
```
...
此脚本会读取当前用户的 `PATH`，检查目标目录是否已经存在；不存在就追加进去，原始值会备份到脚本同目录的 `pathhere.backup.log` 里。

完成后，打开新终端，或者重新调起 `Win + R`，就会生效。
...
脚本改的是“当前用户 PATH”（不是机器级系统 PATH），这不需要管理员权限，也不会影响同一台机器上的其他用户。
...
添加前检查是否已存在，避免重复加入
写入前备份原始用户 PATH
只修改当前用户 PATH，不碰系统 PATH
保留原 Path 注册表值名称和类型
支持空格、中文、&、%、!、括号等路径字符
```

删除时也不是按普通字符串模糊替换，而是把 `PATH` 按分号拆成一个个目录项，再做完整项匹配；所以删除 `C:\Tools`，不会误伤 `C:\ToolsExtra`。如果同一个目录重复出现，删除脚本会把所有匹配项都清掉。

脚本同目录的 `pathhere.backup.log`，里面有每次操作前备份的原始用户 `PATH`，可做最后保障。
