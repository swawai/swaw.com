...
draft: false
title: "VS Code 多窗口，在悄悄共享你的环境变量"
linkTitle: "VS Code 新窗口的环境变量"
slug: "vscode-shared-env"
description: "用 code 打开 VS Code 时，第一次启动所继承的环境变量，可能继续出现在后续窗口中。一次真实复现，以及官方隔离方案之外，我采用的轻量工作流。"
...
> 如果你的 VS Code 默认复用当前窗口，请给第二条命令加上 `--new-window`。

在第二个窗口的终端里跑 `cmd /c set MY_API_KEY`……你会看到啥？它不该在那，但还就是出现了：

![VS Code 意外泄露环境变量](vscode-shared-env.png)


**我测了 Cursor（--classic 模式），也能复现。VS Code 系 IDE 可能都一样！后面只以 VS Code 为例，不再复述。**
...
所以，你要是用带着 API Key 的终端来打开的第一个窗口，那不好意思，Key 就会变成后续所有新建窗口的共享资源。
...
这个行为确实写在文档里了。官方建议，是单独指定 `--user-data-dir`：

```cmd
code D:\proj-a --user-data-dir D:\vscode-data\proj-a
code D:\proj-b --user-data-dir D:\vscode-data\proj-b
```

为了隔离环境变量，搞多个 `user-data-dir`……

这意味着使用不同 `user-data-dir` 的窗口，也有独立的设置、插件、快捷键配置、编辑器历史记录。你每开一个新项目，都要重新装插件、配主题、记哪个窗口是哪套 `user-data-dir`。
...
**最简单的规避方法**：每次 `code .` 之前，若没有其他 VS Code 窗口已开着，从开始菜单先启一个。确保主进程不是由你的 `code .` 拉起的，后面再开多少个窗口都没事。

**包装一个专用脚本，把启动 `code .` 的流程拆成两段**：
...
两套方法，逻辑是一样的：确保主进程已在“别处”启动了。我个人日常用的是脚本方案，一劳永逸；如果你只是偶尔遇到这个问题，记得先开一个空白窗口就够了。
...
4. 这不是什么惊天漏洞，泄露只在你同时开着的多个 VS Code 窗口之间发生，但它足够隐蔽：你在 A 项目配了公司的 OpenAI API Key。切到 B 项目，以为只在用 DeepSeek，某段代码却拿着残留的 Key 跑了起来……月底一看账单：咦，怎么又多了几百刀。
